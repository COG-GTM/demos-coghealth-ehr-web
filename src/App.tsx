    ...
    <>
      ...
      <div className="bg-white border-b border-[#EBEBEB] px-6 py-3 flex items-center justify-between">
        <div className="flex items-center space-x-6">
          <div className="flex items-center space-x-2">
            <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
              <path d="M14 0C6.268 0 0 6.268 0 14s6.268 14 14 14 14-6.268 14-14S21.732 0 14 0z" fill="#FF385C"/>
              <text x="8" y="19" fill="white" fontWeight="bold" fontSize="14" fontFamily="Nunito, sans-serif">C</text>
            </svg>
            <span className="font-bold text-lg text-[#222222] tracking-tight">CogHealth EHR</span>
            <span className="text-xs text-[#717171] font-medium">v4.2.1</span>
          </div>
          {/* Global Patient Search - Airbnb pill style */}
          <div className="relative">
            <div className="airbnb-search">
              <Search className="w-4 h-4 text-[#717171] mr-2 flex-shrink-0" />
              <input
                type="text"
                ...
                className="w-56"
              />
              <button className="airbnb-search-btn ml-2">
                <Search className="w-4 h-4" />
              </button>
            </div>
            {showSearchDropdown && searchResults.length > 0 && (
              <div className="absolute top-full left-0 mt-2 w-80 bg-white rounded-xl border border-[#EBEBEB] shadow-lg z-50 overflow-hidden">
                {searchResults.map((patient) => (
                  <div
                    key={patient.id}
                    onClick={() => selectPatient(patient.id)}
                    className="px-4 py-3 hover:bg-[#F7F7F7] cursor-pointer border-b border-[#F7F7F7] transition-colors"
                  >
                    <div className="font-semibold text-[#222222] text-sm">{patient.name}</div>
                    <div className="text-[#717171] text-xs mt-0.5">{patient.mrn} &bull; DOB: {patient.dob}</div>
                  </div>
                ))}
              </div>
            )}
            {showSearchDropdown && searchResults.length === 0 && globalSearch.length >= 2 && (
              <div className="absolute top-full left-0 mt-2 w-80 bg-white rounded-xl border border-[#EBEBEB] shadow-lg z-50 p-4 text-sm text-[#717171]">
                No patients found
              </div>
            )}
          </div>
        ...
        <div className="flex items-center space-x-4 text-sm">
          <span className="text-[#717171] font-medium">Springfield Medical Center</span>
          <div className="h-4 w-px bg-[#EBEBEB]"></div>
          <div className="flex items-center space-x-1.5">
            <Lock className="w-3.5 h-3.5 text-[#717171]" />
            <span className={`font-medium ${sessionTime < SESSION_WARNING_MS ? 'text-[#FF385C]' : 'text-[#717171]'}`}>
              {formatSessionTime()}
            </span>
          </div>
          <div className="h-4 w-px bg-[#EBEBEB]"></div>
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-[#222222]">Dr. Sarah Anderson</span>
          </div>
          <button onClick={onLogout} className="flex items-center space-x-1.5 text-[#717171] hover:text-[#FF385C] transition-colors px-3 py-1.5 rounded-full hover:bg-[#FFF1F0]">
      ...
      {/* Navigation Toolbar - Airbnb category style */}
      <div className="bg-white border-b border-[#EBEBEB] px-6 flex items-center justify-between">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex flex-col items-center px-4 py-3 border-b-2 transition-all duration-150 ${
                  isActive 
                    ? 'border-[#222222] text-[#222222]' 
                    : 'border-transparent text-[#717171] hover:text-[#222222] hover:border-[#DDDDDD]'
                }`}
              >
                <Icon className={`w-5 h-5 mb-1 ${isActive ? 'text-[#222222]' : ''}`} />
...
function App() {
  ...
  return (
    <BrowserRouter>
      <div className="h-screen flex flex-col" style={{ background: '#ffffff', fontFamily: "'Nunito', system-ui, -apple-system, sans-serif" }}>
        <Navigation 
        ...
        {/* Status Bar - Airbnb minimal style */}
        <div className="bg-[#FAFAFA] border-t border-[#EBEBEB] flex items-center justify-between px-6 py-2 text-xs text-[#717171]">
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-1.5">
              <Shield className="w-3.5 h-3.5 text-[#0D6832]" />
              <span className="font-semibold">HIPAA Compliant</span>
            </div>
            <div className="h-3 w-px bg-[#EBEBEB]"></div>
