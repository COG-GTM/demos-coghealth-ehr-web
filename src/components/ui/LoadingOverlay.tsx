...
export function LoadingOverlay({ isLoading, text = 'Loading...' }: LoadingOverlayProps) {
  ...
  return (
    <div 
      className="ehr-overlay absolute inset-0 z-40 flex items-center justify-center pointer-events-auto"
    >
      <div className="flex flex-col items-center ehr-pane border-2 border-gray-400 dark:border-slate-600 px-6 py-4 shadow-md" style={{ boxShadow: '2px 2px 4px rgba(0,0,0,0.3)' }}>
        <svg
          width="32"
          height="32"
          viewBox="0 0 32 32"
        ...
        <span className="text-[11px] text-gray-700 dark:text-slate-200" style={{ fontFamily: 'Tahoma, sans-serif' }}>{text}</span>
      </div>
    </div>
  );
}
