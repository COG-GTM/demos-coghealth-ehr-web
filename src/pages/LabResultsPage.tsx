...
export default function LabResultsPage() {
  ...
  const getStatusStyle = (status: LabResult['status']) => {
    switch (status) {
      case 'critical':
        return { background: 'var(--ehr-alert-critical-bg)', color: 'var(--ehr-alert-critical-fg)', fontWeight: 'bold' };
      case 'abnormal':
        return { background: 'var(--ehr-alert-warning-bg)', color: 'var(--ehr-alert-warning-fg)' };
      default:
        return {};
    }
  };
          ...
          {filteredPanels.length === 0 ? (
            ...
            filteredPanels.map(panel => (
              <div key={panel.id} className="border-b border-gray-300">
                <div
                  className="flex items-center justify-between px-2 py-1.5 bg-gradient-to-b from-(--ehr-grad-from) to-(--ehr-grad-to) cursor-pointer hover:from-(--ehr-grad-hover-from) hover:to-(--ehr-grad-hover-to)"
                  ...
                  <table className="w-full text-[11px]">
                    ...
                    <tbody>
                      {panel.results.map((result, idx) => (
                        <tr
                          key={result.id}
                          className={`cursor-pointer hover:bg-(--ehr-row-hover) ${idx % 2 === 0 ? 'bg-white' : 'bg-(--ehr-row-alt)'}`}
